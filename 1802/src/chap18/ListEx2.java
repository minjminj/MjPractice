...
public class ListEx2 extends JFrame {
	...
	private JMenuBar getMenuBar_2() {
		if (menuBar == null) {
			...
			menuBar.add(getMnFiel());
		}
		return menuBar;
	}
	private JMenu getMnFiel() {
		if (mnFiel == null) {
			mnFiel = new JMenu("file");
			mnFiel.add(getMenuItem());
			mnFiel.add(getMenuItem_1_1());
		}
		return mnFiel;
	}
	private JMenuItem getMenuItem() {
		if (menuItem == null) {
			menuItem = new JMenuItem("데이터 가져오기");
			menuItem.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					int ync = fc.showOpenDialog(ListEx2.this);
					if (ync == fc.APPROVE_OPTION) {
					...
					try {
						...
						textArea.setText("데이터를 읽었습니다.");
						
					} catch (Exception ex) {
						textArea.setText("파일 읽기 중 오류 발생");
					}
				}
			});
		}
	...
	private JMenuItem getMenuItem_1_1() {
		if (mntmNewMenuItem == null) {
			mntmNewMenuItem = new JMenuItem("데이터 저장하기");
			mntmNewMenuItem.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					...
					if(fileName == null) {
						JFileChooser fc = new JFileChooser();
						int yn = fc.showSaveDialog(ListEx2.this);	// null 자리에 자기자신의 파일명 적어주면 된다. ex ListEx2.this
						if(yn == fc.APPROVE_OPTION) {		// 저장하려는 파일명 선택되지 않음.
							fileName = fc.getSelectedFile().toString();
						} else {
							return;
						}
					...
					try {
						...
						oos.close();
						fos.close();
						textArea.setText("데이터가" + fileName + "에 저장됨.");
					
					} catch (Exception ex) {
						textArea.setText("데이터 저장 중 오류 발생");
					}
				}
			});
		}
